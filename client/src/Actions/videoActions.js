...
export const listAllVideos = () => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get("http://live.vinciis.in/api/videos/");
...
export const getVideoDetails = (videoId) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get(`http://live.vinciis.in/api/videos/${videoId}`);
    dispatch({ 
...
// upload Video
export const uploadVideo = (screenId, campaign) => async(dispatch, getState) => {
  try {
    dispatch({
    ...
    const {data} = await axios.post(`http://live.vinciis.in/api/videos/screen/${screenId}`, {userInfo, campaign}, {
      headers: {
...
export const deleteVideo = (videoId) => async (dispatch, getState) => {
  ...
  try {
    ...
    const { data } = await axios.delete(`http://live.vinciis.in/api/videos/${videoId}`, {
...
export const likeVideo = (videoId, interaction) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`http://live.vinciis.in/api/videos/${videoId}/likeVideo/${interaction}`, { videoId }, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      },
    });
...
export const unlikeVideo = (videoId) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.delete(`http://live.vinciis.in/api/videos/${videoId}/unlikeVideo`, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      },
    });
...
export const flagVideo = (videoId, interaction) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await axios.post(`http://live.vinciis.in/api/videos/${videoId}/flagVideo/${interaction}`, {videoId}, {
      headers:
        { Authorization: `Bearer ${userInfo.token}` }
    });
...
export const viewVideo = (videoId) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`http://live.vinciis.in/api/videos/view/${videoId}`, { videoId }, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      },
    });
...
export const reviewVideo = (videoId, review) => async (dispatch, getState) => {
  ...
  try {
    ...
    const { data } = await axios.post(`http://live.vinciis.in/api/videos/${videoId}/reviews`, review , {
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    });
...
export const updateVideo = (video) => async (dispatch, getState) => {
  ...
  try {
    const { data } = await axios.put(`http://live.vinciis.in/api/videos/${video._id}`, video, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      },
    });
...
export const getVideoParams = (videoId) => async (dispatch) => {
  ...
  try {
    const {data} = await axios.get(`http://live.vinciis.in/api/videos/${videoId}/advertParams`);
    dispatch({
