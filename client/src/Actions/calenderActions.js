...
export const getScreenCalender = (screenId) => async(dispatch, getState) => {
  ...
  try {
    const {data} = await Axios.get(`http://live.vinciis.in/api/calender/screen/${screenId}/slots`, {
      headers: {
...
export const addCalenderData = (screenId, calender) => async(dispatch, getState) => {
  ...
  try {
    ...
    const {data} = await Axios.put(`http://live.vinciis.in/api/calender/screen/${screenId}`, calender, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      }
    });
...
export const addCalenderDayData = (screenId, calender) => async(dispatch, getState) => {
  ...
  try {
    ...
    const {data} = await Axios.put(`http://live.vinciis.in/api/calender/screen/${screenId}/day`, calender, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      }
    });
...
export const bookDaySlot = (screenId, daySlotToBook ) => async(dispatch, getState) => {
  ...
  const {userSignin: {userInfo}} = getState();
  try {
    const {data} = await Axios.post(`http://live.vinciis.in/api/calender/screen/${screenId}/day`, {daySlotToBook}, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      }
    });
...
export const bookSlot = (screenId, slotId, slotToBook) => async(dispatch, getState) => {
  ...
  try {
    const {data} = await Axios.put(`http://live.vinciis.in/api/calender/screen/${screenId}/slot/${slotId}/booking`, slotToBook,{
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      }
    });
...
export const bookDay = (screenId, dayId, dayToBook) => async(dispatch, getState) => {
  ...
  try {
    const {data} = await Axios.put(`http://live.vinciis.in/api/calender/screen/${screenId}/day/${dayId}/booking`, dayToBook,{
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      }
    });
