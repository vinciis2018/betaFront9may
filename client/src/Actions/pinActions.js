...
export const getAllPins = () => async(dispatch) => {
  ...
  try {
    const {data} = await axios.get('http://3.7.140.85:3333/api/pins');
    dispatch({
...
export const getPinJson = () => async (dispatch) => {
  ...
  try {
    const {data} = await axios.get('http://3.7.140.85:3333/api/pins/allPinGeoJson');
    dispatch({
...
export const getPinDetails = (pinId) => {
  return async (dispatch) => {
    ...
    try {
      const {data} = await axios.get(`http://3.7.140.85:3333/api/pins/${pinId}`);
      dispatch({
...
export const addPins = (category, lat, lon) => async(dispatch, getState) => {
  ...
  try {
    const {data} = await axios.post('http://3.7.140.85:3333/api/pins', {category, lat, lon}, {
      headers: {
...
export const updatePin = (screenId, pin) => async (dispatch, getState) => {
  ...
  const { userSignin: { userInfo } } = getState();
  try {
    const {data} = await axios.put(`http://3.7.140.85:3333/api/pins/${screenId}`, pin, {
      headers: {
        Authorization: `Bearer ${userInfo.token}`
      },
    });
